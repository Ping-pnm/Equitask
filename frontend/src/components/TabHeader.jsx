    ...
    return (
        <header className="top-nav">
            <nav className="tabs">
                ...
                <Link to="/dashboard" className={`tab-item ${location.pathname === "/dashboard" || location.pathname.startsWith('/project/') || location.pathname.startsWith('/group-project/') || location.pathname.startsWith('/task/') || location.pathname.startsWith('/leader-assignment/') ? "active" : ""}`}>Dashboard</Link>
            </nav>
        </header>
    );
}